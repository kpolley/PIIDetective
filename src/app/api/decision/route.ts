...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!columnId || !decision) {
            return new NextResponse(JSON.stringify({ error: 'Column ID and decision is required' }), {
                status: 400,
        ...
        if (decision !== 'accept' && decision !== 'reject') {
            return new NextResponse(JSON.stringify({ error: 'Decision must be either "accept" or "reject"' }), {
                status: 400,
                headers: { 'Content-Type': 'application/json' },
            });
        }
        ...
        return new NextResponse(JSON.stringify({ message: 'Policy tag applied', columnId }), {
            headers: { 'Content-Type': 'application/json' },
        });
    } catch (error) {
