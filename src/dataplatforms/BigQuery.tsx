...
export class BigQueryPlatform extends DataPlatform {
    ...
    async *getDatasetTables(datasetId: string): AsyncGenerator<TableDataType> {
        const [tables]: GetTablesResponse = await bigQueryClient.dataset(datasetId).getTables();

        for (const table of tables) {
            const schema: TableSchema | undefined = await table.getMetadata()
            ...
            yield {
                ...
                columns: schema.fields
                    .filter((field): field is { name: string } => field.name !== undefined)
    ...
    async applyPolicyTag(datasetId: string, tableName: string, columnName: string, policyTagId: string): Promise<void> {
        ...
        const schema: TableSchema = await tableRef.getMetadata()
            .then(([data]: TableMetadata[]) => data.schema);
