...
import { useColumnQuery, ColumnClassificationIncludeColumn } from "@/app/providers"
import { useMutation } from "@tanstack/react-query";

export default function Component({ columnItem }: { columnItem: ColumnClassificationIncludeColumn | null }) {
    ...
    return (
        <Card>
            ...
            <CardFooter className="flex space-x-2">
                <LoadingButton onClick={() => callDecisionApi({ columnId: columnItem.columnId, decision: "accept" } as DecisionAPIBody)} className="w-full">Accept</LoadingButton>
                <LoadingButton onClick={() => callDecisionApi({ columnId: columnItem.columnId, decision: "reject" } as DecisionAPIBody)} variant="destructive" className="w-full">Reject</LoadingButton>
            </CardFooter>
        </Card>
    )
}